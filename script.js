...
const noInfo = `Информация в технологическом регламенте не была найдена`;
...
async function updateContent() {
  try {
    ...
    valveNumber.textContent = valve.number;

    for (let i = 0; i < valveTextHeadings.length; i++)
      valveTextHeadings[i].textContent = valveHeadings[i];

    valveLocation.textContent = `${valve.location}`;
    valveUse.textContent = `${valve.use}`;
    valveType.textContent = `${valve.type}`;
    valveShutdownReason.textContent = `${valve.shutdownReason}`;
    valve.important
...
function toggleLoadingPlaceholder() {
  if (valveImageWrapper.classList.contains("img-wrapper_theme-loading")) {
    valvePhoto.classList.remove("valve-photo_theme-loading");
    valveImageWrapper.classList.remove("img-wrapper_theme-loading");
  } else {
    valvePhoto.classList.add("valve-photo_theme-loading");
    valveImageWrapper.classList.add("img-wrapper_theme-loading");
  }
}
